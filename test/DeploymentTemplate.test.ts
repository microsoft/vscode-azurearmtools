...
suite("DeploymentTemplate", () => {
    ...
    suite("findParameterDefinitionsWithPrefix(string)", () => {
        ...
        test("with case sensitive and insensitive match", () => {
            const dt = new DeploymentTemplateDoc("{ 'parameters': { 'apples': { 'type': 'string' }, 'APPLES': { 'type': 'integer' } } }", fakeId);
    ...
    suite("findVariableDefinitionsWithPrefix(string)", () => {
        test("with undefined", () => {
            const dt = new DeploymentTemplateDoc("{ 'variables': { 'apples': 'APPLES', 'bananas': 88 } }", fakeId);
            // tslint:disable-next-line:no-any
