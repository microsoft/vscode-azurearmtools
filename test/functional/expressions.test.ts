...
import { armToolsSource, IDeploymentTemplate, testDiagnostics } from "../support/diagnostics";

suite("Expressions functional tests", () => {
    function testExpression(testName: string, expression: string, expected: string[]): Promise<void>;
    function testExpression(expression: string, expected: string[]): Promise<void>;

    async function testExpression(testNameOrExpression: string, expressionOrExpected: string | string[], expected?: string[]): Promise<void> {
        let testName: string;
        let expression: string;

        if (typeof expressionOrExpected === "string") {
            // (testName, expression, expected)
            testName = testNameOrExpression;
            expression = expressionOrExpected;
        } else {
            expression = testNameOrExpression;
            testName = expression;
            expected = expressionOrExpected;
        }
